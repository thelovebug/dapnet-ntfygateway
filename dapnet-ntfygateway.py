...
if __name__ == '__main__':
    ...
    while True:
        ...
        filename = f'{settings["logfile"]["path"]}{settings["logfile"]["format"]}'.replace(
            "{date}", current_date)

        # Send informational message that we're waiting for the logfile
        func_messages.info_message(
            "logfile_waiting", settings, optional=filename)
        ...
        func_messages.info_message(
            "logfile_monitoring", settings, optional=filename)
